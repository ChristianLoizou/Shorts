...
class Application(Tk):
    ...
    def add_widgets(self):
        ...
        self.exportmenu.add_command(
            label='Export to TXT', command=lambda: self.export('txt'))
        self.exportmenu.add_command(
            label='Export to MIDI', command=lambda: self.export('midi'))
        self.exportmenu.add_command(
            label='Export to MXL', command=lambda: self.export('mxl'))
        ...
        self.btn_run = Button(
            self.mainframe, text='Generate sequence', command=self.execute)
        self.btn_copy = Button(
            self.mainframe, text='Copy output', command=self.copy_output)

        self.text_output.pack(expand=True, fill='both', padx=15, pady=15)
        self.lblframe_outputframe.grid(
            row=0, column=0, rowspan=3, columnspan=4, padx=15, pady=15)
    ...
    def settings_popup(self):
        messagebox.showinfo(
            title="WiP", message="Limited settings are available as of yet. More are coming soon! Stay tuned...")
        ...
        accidental_types_menu = Menu(
            self.settings_widgets['accidental_type_menu'], tearoff=False)
        ...
        for fn, frame in enumerate(self.settings_frames.values()):
            frame.grid(row=fn, rowspan=1, column=0, columnspan=3,
    ...
    def change_setting(self, setting, value):
        ...
        if setting == 'accidental_type':
            ...
            self.settings_widgets['accidental_type_menu'].configure(
            ...
            self.settings_widgets['key_menu']['menu'] = key_menu
            enharm_equiv = get_enharmonic_equivalent(
                self.SETTINGS['key'].get())
            ...
            self.SETTINGS['max_repetitions'].set(
                self.settings_widgets['max_repetitions_spin'].get())
        elif setting == 'mode':
            self.settings_widgets['mode_menu'].configure(
    ...
    def information_popup(self):
        messagebox.showwarning(title="Feature unavailable",
                               message="This feature hasn't been implemented yet. Please check back later!")
        pass
...
def createMXL(output):
    ...
    chords = [list(chain(chord, ['r', ]*(num_voices-len(chord))))
              for chord in pre_chords]
    ...
    for measure_number, _chord in enumerate(chords):
        for voice, _note in enumerate(_chord):
            ...
            if pitch == 'r':
                nn = note.Rest(type='whole')
            else:
                nn = note.Note(f'{pitch}{octave}', type='whole')
            measure = stream.Measure(number=measure_number+1)
            measure.append(nn)
            parts[voice].append(measure)
    for part in parts:
        _score.insert(0, part)
    return _score
...
def get_num_voices(vs):
    l = vs.split('-')
    if len(l) > 1:
        return random.randint(*sorted(list(map(int, l))))
    else:
        return int(l[0])
