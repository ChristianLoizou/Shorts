...
def draw_exercise(exercise, canvas):
    ...
    if OPTIONS['SHOW_GUIDELINES'].get():
        ...
        turt.penup()
        turt.speed(-1)
        ys = set([Interval(interval, ym).get_dy() for interval in INTERVALS for ym in [-1, 1]])
        _turtles = list()
...
def settings():
    ...
    homenote_om = OptionMenu(other_frame, OPTIONS['HOME_NOTE_VARIABLE'], *NOTE_NAMES, command=update_starting_note)
